	...
	public function netmountPrepare($options) {
        ...
        if ($options['user'] === 'init') {
			...
			if ($options['pass'] === 'init') {
				$html = '';
				if ( isset($session->elFinderDropboxTokens) ) {
					// token check
					try {
						list(, $accessToken, $accessTokenSecret) = $session->elFinderDropboxTokens;
                        ...
                        unset($session->elFinderDropboxTokens);						
					}
				}
				if (! $html) {
					// get customdata
					...
					$session->elFinderDropboxAuthTokens = $tokens;
				...
				$this->oauth->setToken($session->elFinderDropboxAuthTokens);
				unset($session->elFinderDropboxAuthTokens);
				$tokens = $this->oauth->getAccessToken();
				$session->elFinderDropboxTokens = array($_GET['uid'], $tokens['token'], $tokens['token_secret']);
		...
		if (isset($session->elFinderDropboxTokens)) {
			list($options['dropboxUid'], $options['accessToken'], $options['accessTokenSecret']) = $session->elFinderDropboxTokens;
		}
